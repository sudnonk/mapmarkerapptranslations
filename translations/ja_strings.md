# Map Marker translations for Japanese (ja)
...
translated (by machine 🤖) as:
```
保存先のディレクトリにアクセスできませんでした。お使いの端末がコンピュータにUSBケーブルで繋がっていないことを確認し、アプリを再起動してください。
```
...
translated (by machine 🤖) as:
```
本当にこのフォルダを削除しても良いですか？このフォルダに関連付いたすべてのマーカーは完全に削除されます！
```
...
translated (by machine 🤖) as:
```
そのフォルダは既に存在するため、新しいフォルダを作成できませんでした。
```
...
translated (by machine 🤖) as:
```
そのフォルダ名のフォルダは既に存在するため、フォルダの名前を変更できませんでした。
```
...
translated (by machine 🤖) as:
```
このスナップショットは、Map Markerで生成されました：
 %s
```
...
translated (by machine 🤖) as:
```
内部タスクマネージャの起動時にエラーが発生しました。アプリを終了します。
```
...
translated (by machine 🤖) as:
```
Google Play Servicesに問題があります：それがインストール、更新または有効化されている必要があります。解決を試みるには、[OK]をクリックしてください。
```
...
translated (by machine 🤖) as:
```
もしこのアプリを気に入ったら、良い評価を与えていただけると開発者が喜びます :-)
```
...
translated (by machine 🤖) as:
```
マーカーを配置したい場所に地図を動かして、OKをクリックして設置します。
```
...
translated (by machine 🤖) as:
```
&lt;！[CDATA [&lt;HTML&gt; &lt;BODY&gt; &lt;B&gt;方位を調整するために、端末を校正する必要があります。&lt;BR&gt;校正するには、単純に8を描くように端末を大きく動かしてください。大きく動かし、それを約3回繰り返すことでより精度が上がります。&lt;BR&gt;校正は、これをすることでいつでも行うことができます。&lt;/ b&gt;の&lt;br&gt;校正には限界があり、また以下のような場所の近くでは十分な精度にならないことがあります。&lt;BR&gt;&amp;bull; 大きな金属の物体 &lt;BR&gt;&amp;bull;電子機器&lt;BR&gt;&amp;bull;磁石&lt;BR&gt;&amp;bull;車、バス、電車、飛行機の中&lt;/BODY&gt;&lt;/HTML&gt;]]&gt
```
...
translated (by machine 🤖) as:
```
ご寄付いただきありがとうございます！このバッジの購入により、あなたはプレミアム機能を使えるようになりました :-)
```
...
translated (by machine 🤖) as:
```
プレミアム機能が解放されました！プレミアム機能を使うことができるようになりました。
```
...
translated (by machine 🤖) as:
```
そのフォルダには、%1$s というファイル名のファイルが複数入っているため、選択できません。同期のためにはそのフォルダにはその名前のファイルが1つしか入っていないことが必要です。
```
...
translated (by machine 🤖) as:
```
そのフォルダ内には同期のためのデータがすでに存在しているようです。もしこのフォルダを選択した場合、同期時にローカルのデータはそのフォルダのデータにまとめられます。本当にこうして良いですか？
```
...
translated (by machine 🤖) as:
```
Map Markerプレミアムアップグレードをお買い上げいただき、誠にありがとうございます！ :-)
```
...
translated (by machine 🤖) as:
```
無料のMap Marker Premiumを要求することで興味を示してくれてありがとうございます。 :-)
```
...
translated (by machine 🤖) as:
```
プレミアムにアップグレードして、Map Markerの機能を最大限に活用しましょう！
```
...
translated (by machine 🤖) as:
```
申し訳ありませんが、この機能にアクセスするにはプレミアムへのアップグレードが必要です。プレミアムにアップグレードして、Map Markerの機能を最大限に活用しましょう！
```
...
translated (by machine 🤖) as:
```
あなたは無料のプレミアムアップグレードを持っています！もしよりサポートしたい場合は、有料で購入することもできます。 :-))
```
...
translated (by machine 🤖) as:
```
私のGoogleアカウントはすでにプレミアムを持っているので、それを反映します！
```
...
translated (by machine 🤖) as:
```
問題の原因を解決できるかもしれないアクションのリストを開きます。
```
...
translated (by machine 🤖) as:
```
アプリが内蔵のSDカードにファイルを保存できるかどうかをチェック
```
...
translated (by machine 🤖) as:
```
現在インストールされているGoogle Play Servicesアプリの詳細を開く
```
...
translated (by machine 🤖) as:
```
ディスクのテストに失敗しました：外部ストレージに書き込むことができませんでした
```
...
translated (by machine 🤖) as:
```
クラウドマップフォルダを開いて、詳細を見たり他の人と共有したりします
```
...
translated (by machine 🤖) as:
```
canは、次の拡張子を持つファイルのみをインポートします：%s
```
