...
translated (by machine 🤖) as:
```
保存先ディレクトリにアクセスできませんでした。お使いのデバイスがUSBケーブルによってコンピュータに接続されていないことを確認の上、アプリを再起動してください。
```
...
translated (by machine 🤖) as:
```
本当にこのフォルダを削除しますか？このフォルダに関連付けられた全てのマーカーが完全に削除されます！
```
...
translated (by machine 🤖) as:
```
新しいフォルダを作成できませんでした。このフォルダは既に存在しています。
```
...
translated (by machine 🤖) as:
```
フォルダの名前を変更できませんでした、変更先の名前のフォルダが既に存在します。
```
...
translated (by machine 🤖) as:
```
位置情報サービスが無効になっています。現在値を取得するには、それらを有効にする必要があります。
```
...
translated (by machine 🤖) as:
```
Map Marker上のマーカーのスナップショットを取得する
```
...
translated (by machine 🤖) as:
```
このスナップショットは、Map Markerアプリで作成されました： 
 %s
```
...
translated (by machine 🤖) as:
```
マーカーの場所に移動する際に、マップ上のカメラの位置を調整する
```
...
translated (by machine 🤖) as:
```
内部タスクマネージャを起動時にエラーが発生しました。アプリを終了します。
```
...
translated (by machine 🤖) as:
```
Google Play Servicesの最新版がインストールされ、有効化されている必要があります。[OK]をクリックして解決を試みます。
```
...
translated (by machine 🤖) as:
```
このアプリを気に入りましたか？このアプリの評価のため少しお時間をください :-)
```
